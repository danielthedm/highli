import { filterActive, activeNames, activeContext, type Source } from "../registry.js";

export function buildSystemPrompt(
  sources: Source[],
  timeframe?: { from: string; to: string },
): string {
  const sourceNames = activeNames(sources);
  const sourceContext = activeContext(sources);
  ...
  const sourceDescriptions = filterActive(sources)
