...
import { getConfig } from "@highli/core";
import { defineSource, getSourceMethod } from "@highli/core";
import { formatSourceResult, type SourceResult } from "@highli/core";
import { claudeMcpQuery } from "@highli/core";
import { getTargetUser } from "@highli/core";
...
async function ingestRange(since: string, until: string): Promise<Event[]> {
  const octokit = getClient();
  const username = await getUsername(octokit);
  const scope = buildScopeFilter();
  const events: Event[] = [];

  const searchAuthored = `author:${username} created:${since}..${until} is:pr ${scope}`.trim();
  let page = 1;
  while (true) {
    const { data } = await octokit.search.issuesAndPullRequests({
      q: searchAuthored,
    ...
    for (const pr of data.items) {
      ...
      events.push({
        id: `github:pr-authored:${repo}#${pr.number}`,
        source: "github",
        type: "pr_authored",
        ts: new Date(pr.created_at).getTime(),
        title: pr.title,
        summary: `#${pr.number} in ${repo} — ${pr.state}${
          pr.pull_request?.merged_at ? " (merged)" : ""
        }`,
        url: pr.html_url,
        payload: {
          number: pr.number,
  ...
  const searchReviewed = `reviewed-by:${username} created:${since}..${until} is:pr ${scope}`.trim();
  page = 1;
  while (true) {
    const { data } = await octokit.search.issuesAndPullRequests({
      q: searchReviewed,
    ...
    for (const pr of data.items) {
      const repo = repoFromUrl(pr.repository_url);
      events.push({
        id: `github:pr-reviewed:${repo}#${pr.number}:${username}`,
        source: "github",
        type: "pr_reviewed",
        ts: new Date(pr.created_at).getTime(),
        title: `Reviewed: ${pr.title}`,
        summary: `#${pr.number} in ${repo}`,
        url: pr.html_url,
        payload: {
          number: pr.number,
          repo,
  ...
  const commitQuery = `author:${username} committer-date:${since}..${until} ${scope}`.trim();
  page = 1;
  while (true) {
    const { data } = await octokit.search.commits({
      q: commitQuery,
    ...
    for (const commit of data.items) {
      ...
      const ts = commit.commit.committer?.date
        ? new Date(commit.commit.committer.date).getTime()
        : Date.now();
      events.push({
        id: `github:commit:${repo}@${commit.sha}`,
        source: "github",
        type: "commit",
        ts,
        title: commit.commit.message.split("\n")[0],
